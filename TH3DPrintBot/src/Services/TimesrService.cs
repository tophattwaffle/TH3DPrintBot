    ...
    public class TimerService : ITimerService
    {
        ...
        public TimerService(DiscordSocketClient client, DataService data, Random rand)
        {
            ...
            _timer = new Timer(
                ...
                null,
                TimeSpan.FromSeconds(_data.RootSettings.program_settings.timerStartDelay),
                TimeSpan.FromSeconds(_data.RootSettings.program_settings.timerUpdateInterval));
        }
        ...
        public void Start()
        {
            ...
            _timer.Change(TimeSpan.FromSeconds(_data.RootSettings.program_settings.timerStartDelay), TimeSpan.FromSeconds(_data.RootSettings.program_settings.timerUpdateInterval));
            Running = true;
            Console.WriteLine($"Timer started with an interval of {_data.RootSettings.program_settings.timerUpdateInterval} seconds.\n");
        }
        ...
        private async Task ChangePlayingAsync() =>
            await _client.SetGameAsync(_data.RootSettings.lists.playing.ElementAt(_random.Next(0, _data.RootSettings.lists.playing.Count)));
    }
}
